...
module.exports = function(group, element, statusModelElement, bpmnFactory) { 
    ...
    validate: function(element, values, node) { 
      ...
      if (formField) { 
        ...
        if (!idValue || idValue.trim() === '') { 
          return { id: __('Form field id must not be empty') };
        }
        ...
        if (existingFormField) { 
          return { id: __('Form field id already used in form data.') };
        }
      }
    }
  }));
  ...
  group.entries.push(entryFactory.comboBox({ 
    ...
    set: function(element, values, node) { 
      ...
      if (selectedFormField.type === 'status' && values.type !== 'status') { 
        // delete camunda:value objects from formField.values when switching from type enum
        commands.push(cmdHelper.updateBusinessObject(element, selectedFormField, { values: undefined }));
      }
  ...
  group.entries.push(entryFactory.statusTable({ 
    statusModelElement: statusModelElement,
    ...
    fillElements: function(element, node) { 
      var selectedFormField = getSelectedFormField(element, node),
          statuses = [],
          values = [],
          elementSM = statusModelElement ? statusModelElement.element : undefined;

      if (!!elementSM && !!elementSM.businessObject &&
        !!elementSM.businessObject.extensionElements) { 
        var subvalues = elementSM.businessObject.extensionElements.values;
        if (!!subvalues && subvalues.length > 0 && !!subvalues[0].fields && subvalues[0].fields.length > 0 &&
          !!subvalues[0].fields[0].values) { 
          statuses = subvalues[0].fields[0].values;
        }
      }
      var bo = getBusinessObject(element);
      for (var i = 0; i < statuses.length; i++) { 
        var status = statuses[i],
            enumValue = elementHelper.createElement('camunda:Value', { id: status.id, name: status.name },
            bo, bpmnFactory);
        values.push(enumValue);
      }
      return cmdHelper.addElementsTolist(element, selectedFormField, 'values', values);
    },
  ...
  group.entries.push(entryFactory.table({ 
    ...
    labels: [__('Name'), __('Config')],
    addLabel: __('Add Constraint'),
    getElements: function(element, node) { 
      var formField = getSelectedFormField(element, node);
